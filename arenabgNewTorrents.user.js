// ==UserScript==
// @name       ArenaBG new Top10 elements
// @namespace  http://begg.eu
// @version    0.2
// @description   ArenaBg changed top10 By Peter Denev
// @match      http://arenabg.com/torrents/
...
function getFirstNewHref(){
	return document.getElementsByClassName('torrents')[0]
...
if(window.location.href.substr(0, 28) == "http://arenabg.com/torrents/" )
{

	var loc = window.location.href;	
	var ifrId = 0;
	var battle_code = loc.substring(loc.indexOf('battlefield/')+12);
	var cookie_top10_td = 'arenabgTop10_pd';
    var cookie_last_top_row = 'arenabgLastTopRow_pd';
    var cookie_hours = 48;
    ...
    function getCookieLastTopRow(){
		var cookieLastTopRow = readCookie(cookie_last_top_row); //eval('(' + readCookie(cookie_top10_td) + ')');
		if(cookieLastTopRow==null) cookieLastTopRow = "";
		return cookieLastTopRow;
	}
	
	function setCookieLastTopRow(a_href){		
		createCookie(cookie_last_top_row,a_href,cookie_hours);				
	}  


	function init(){		
		var top10TRs = document.getElementById('main').getElementsByTagName('table')[0].getElementsByTagName('tr');
		...
		for(var i=1;i<top10TRs.length;i++){
			...
			var found = false;		
			for(var j=0;j<10;j++){				
				var cookie_el = getCookieTop(j);				
				if(cookie_el==Base64.encode(cur_el.innerHTML)){					
					found = true;
				}
			}			
			if(!found){				
				cur_el.style.backgroundColor="#044F15";
			}
    ...
    function init2(){
        ...
        if(old_a_href!=''){
            ...
            for(i=1;i<=torr_rows.length;i++){
                ...
                if(temp_href==old_a_href){
                    ...
                    temp_td.style.backgroundColor="#044F15";
                }
            }
        ...
        setCookieLastTopRow(getFirstNewHref());        
    }
